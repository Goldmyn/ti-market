    ...
    <div className="max-w-6xl mx-auto p-2">
        <nav className="flex justify-between">
            <div className="flex items-center space-x-2 text-sm">
                {address ? (
                    <button onClick={disconnect}            className='connectWalletbtn'>Hi, {address.slice(0, 4) + "..." + address.slice(-4)}
                    </button>
                ) : (
                <button onClick={connectWithMetamask} className='connectWalletbtn'>Connect our wallet</button>
                )}
        ...
        <section className="flex items-center space-x-2 py-5">
            ...
            <button className="hidden sm:inline bg-blue-600 text-white px-5 md:px-10 py-2 border-2 rounded-b-lg border-blue-600">
                Search
            </button>

            <Link href="/create">
                <button className="border-2 border-blue-600 px-5 md:px-10 py-2 text-blue-600 rounded-b-lg hover:bg-blue-600/50 hover:text-white cursor-pointer">
                    List Item
                </button>
            </Link>
        </section>
